	...
	public abstract class AbstractFileTabControlHandler
	{
			...
			where TAssetRowControl : UserControl
		{
			for (int i = 0; i < assetTabControlHandler.Assets.Count; i++)
			{
				...
				TUserAsset userAsset = userAssets.FirstOrDefault(a => a.AssetName == asset.AssetName && (asset.ChunkTypeName == null /*particles don't have a chunk type*/ || a.ChunkTypeName == asset.ChunkTypeName));
				if (userAsset != null)
				{
